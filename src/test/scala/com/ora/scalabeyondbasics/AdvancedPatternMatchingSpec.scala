...
class AdvancedPatternMatchingSpec extends FunSpec with Matchers {
  ...
  describe("Simple Pattern Matches") {
    ...
    it(
      "can be used with an Optional, and often is used as such, let's do a Some") {
      val Some(x) = Some(100)
      x should be(100)
    }
    ...
    it("""Lets do up a replicate method""".stripMargin) {
      ...
      def Replicate[A](Count: Int, Value: A) =
        (1 to Count).map(_ => Value).toList

      Replicate(0, "Whoa") should be(List())
      Replicate(1, "Whoa") should be(List("Whoa"))
      Replicate(2, "Whoa") should be(List("Whoa", "Whoa"))
      Replicate(5, "Whoa") should be(
        List("Whoa", "Whoa", "Whoa", "Whoa", "Whoa"))
    }
    ...
    it("""should show an empty list because we haven't covered that yet.""") {
      ...
      def MySecond[A](list: List[A]): Option[A] = list match {
      ...
      MySecond(List("a", "b", "c", "d", "e", "f")) should be(Some("b"))
    }
  ...
  describe("A Regular Pattern Expression Match") {
    it(
      """uses .r after a String to Convert it to a Regex Type, from there groups can can be determined""".stripMargin
    ) {
      pending
    }
  }
